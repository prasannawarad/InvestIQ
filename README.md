...
| **2 — Kuber** | **Shipped:** `packages/kuber` Groq streams + **`serialize-context`/`demo-responses`** (persona/tone lock-in in context), **system prompt** `kuber-agent-system.ts` (adapted from `agent/recommendation_agent.py`). **`/api/kuber/*`**, floating Kubers, **`/Kuber`** discovery: filters → **`POST /api/engine/matches`** → basket → **`Review & Confirm`** persists mock-universe **`holdings`** + **`portfolios`** JSON via `applyDiscoveryCommit` (Groq unused on `/Kuber` rankings — engine-only). **`GROQ_API_KEY`** + **`NEXT_PUBLIC_*`** Supabase cookies. |
